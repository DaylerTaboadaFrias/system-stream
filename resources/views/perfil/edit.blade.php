        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="overflow-x-auto relative shadow-md sm:rounded-lg">
                    ...
                    @endif
                    <form method="POST" action="{{ route('perfil.update', [$perfil->id]) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                            ...
                            <div class="relative mb-4">
                                ...
                                <input type="date" id="fecha_compra" name="fecha_compra"
                                    value="{{ old('fecha_compra', \Carbon\Carbon::parse($perfil->fecha_compra)->format('Y-m-d') ) }}"
                                    class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                @error('fecha_compra')
                                    <p id="standard_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400"><span class="font-medium">{{ $message }}</span></p>
                                @enderror
                            ...
                            <div class="relative mb-4">
                                ...
                                <input type="date" id="fecha_vencimiento" name="fecha_vencimiento"
                                    value="{{ old('fecha_vencimiento', \Carbon\Carbon::parse( $perfil->fecha_vencimiento)->format('Y-m-d')) }}"
                                    class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                            ...
                            <div class="relative mb-4">
                                ...
                                <select id="id_plataforma" name="id_plataforma"
                                    ...
                                    @foreach ($perfiles as $perfil)
                                        <option
                                            {{ (int) old('id_plataforma', $perfil->id_plataforma) === $perfil->id ? 'selected' : '' }}
